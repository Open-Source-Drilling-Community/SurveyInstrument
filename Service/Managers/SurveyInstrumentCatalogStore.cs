...
internal sealed class SurveyInstrumentCatalogStore<T> where T : class
{
    ...
    public SurveyInstrumentCatalogStore(SqlConnectionManager manager, string table, string documentColumn,
    ...
    public bool Add(T value)
    {
        ...
        if (info == null || info.ID == Guid.Empty || ById(info.ID) != null)
        {
            return false;
        }
        DateTimeOffset now = DateTimeOffset.UtcNow;
        setCreated(value, now);
        setModified(value, now);
        return Write(value, false);
    }

    public bool Update(Guid id, T value)
    {
        if (id == Guid.Empty || meta(value)?.ID != id || ById(id) == null)
        {
            return false;
        }
    ...
    private bool Write(T value, bool update)
    {
        ...
        if (exclusive == null)
        {
            command.CommandText = update
                ...
                : $"INSERT OR IGNORE INTO {table}(ID,MetaInfo,Name,CreationDate,LastModificationDate,{documentColumn}) VALUES($id,$meta,$name,$created,$modified,$document)";
        }
        else
        {
            command.CommandText = update
                ...
                : $"INSERT OR IGNORE INTO {table}(ID,MetaInfo,Name,IsExclusive,HasValidityPeriod,CreationDate,LastModificationDate,{documentColumn}) VALUES($id,$meta,$name,$exclusive,$validity,$created,$modified,$document)";
        }
    ...
    private static object? ReadDate(T value, string property) =>
        value.GetType().GetProperty(property)?.GetValue(value) is DateTimeOffset date ? date.ToString("O") : null;
}
