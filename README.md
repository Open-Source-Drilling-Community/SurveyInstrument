...
The checked-in service publishes 26 domain MCP tools covering Survey Instrument, Error Source, identity-catalog, and feature-category operations, together with `ping`. Usage-statistics endpoints are intentionally excluded. The extra MCP-only `survey_instrument_patch_by_id` operation provides guarded partial updates without changing the REST surface.
...
The tools provide strict input schemas and explicit success-output schemas for full Survey Instrument and Error Source payloads, identity and feature catalogs, and embedded assignments. Survey Instrument schemas use `ModelType` as a documented discriminator with guidance for each family. Every tool publishes a human-readable title and MCP safety annotations. Successes provide structured JSON plus text fallback; validation, not-found, conflict, stale-write, and unexpected server failures produce stable sanitized MCP error envelopes. Core Survey Instrument writes and catalog writes require optimistic-concurrency timestamps. Referenced identity/feature definitions cannot be removed. The standalone Error Source store is explicitly a template library: instruments own embedded snapshots, and template changes never silently rewrite an existing instrument. Physical inputs and outputs use SI: angles are radians, distances are metres, gravity is m/s², magnetic flux density is tesla, and error-source `Magnitude` is expressed in the SI unit identified by `MagnitudeQuantity`.
