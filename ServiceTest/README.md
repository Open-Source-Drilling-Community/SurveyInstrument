...
- `McpToolRegistrationTests.cs` verifies all 27 REST-backed tools, the MCP-only guarded patch and drift-check tools, and `ping`, including strict input/output schemas, enforcing model discrimination, the complete error-code enum, versioned backup contracts, core-write concurrency, titles, safety annotations, and pre-invocation rejection of unknown arguments.
